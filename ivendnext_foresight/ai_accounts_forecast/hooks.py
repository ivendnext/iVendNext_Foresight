...
def on_financial_forecast_save(doc, method):
    ...
    try:
        ...
        try:
            from ivendnext_foresight.forecasting.sync_manager import sync_single_forecast
            frappe.enqueue(
...
def enqueue_forecast_update(company, account, trigger_source):
    ...
    try:
        ...
        if not frappe.cache().get(queue_key):
            ...
            frappe.enqueue(
                'ivendnext_foresight.ai_accounts_forecast.jobs.update_account_forecast',
                company=company,
                account=account,
...
def enqueue_initial_forecast_creation(company, account, account_type):
    ...
    try:
        ...
        for forecast_type in forecast_types:
            frappe.enqueue(
                'ivendnext_foresight.ai_accounts_forecast.jobs.create_initial_forecast',
                company=company,
                account=account,
...
def update_account_forecast(company, account, trigger_source):
    ...
    try:
        from ivendnext_foresight.ai_accounts_forecast.models.account_forecast import create_financial_forecast
